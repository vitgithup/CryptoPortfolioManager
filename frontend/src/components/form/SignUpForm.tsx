...
import * as PortfoliosApi from '../../network/api';
...
const SignUpForm = () => {
  ...
  const handleSubmit = async (ev: { preventDefault: () => void; }) => {
    ev.preventDefault();
    try {
      const response = await PortfoliosApi.signUp({ username, password })
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <p className="text-sm font-light text-gray-500 ">
                Already have an account? <Link href='/sign-in' className="font-medium text-primary-600 hover:underline text-lg text-blue-500">Login</Link>
